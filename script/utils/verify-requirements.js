...
function verifyNode(cb) {
  ...
  if (nodeMajorVersion === 0 && nodeMinorVersion < 10) {
    error = "node v0.10 is required to build Glue, node " + nodeVersion + " is installed.";
    cb(error);
  }
  else {
...
function verifyNpm(cb) {
  ...
  childProcess.execFile(npmCommand, ['-v'], { env: process.env }, function(err, stdout) {
    if (err)
      return cb("npm 1.4 is required to build Glue. An error (" + err + ") occured when checking the version.");
...
function checkPythonVersion (python, cb) {
  ...
  childProcess.execFile(python, ['-c', 'import platform; print(platform.python_version());'], { env: process.env }, function (err, stdout) {
    if (err) {
      error = "Python 2.7 is required to build Glue. An error (" + err + ") occured when checking the version of '" + python + "'. ";
    ...
    if (!goodPythonVersion) {
      error = "Python 2.7 is required to build Glue. '" + python + "' returns version " + version + ". ";
      error += pythonHelpMessage;
      cb(error);
      return;
    }
